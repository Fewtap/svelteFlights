...
export function converttimes(flight: IFlight) {
		...
		flight.planned = moment.utc(flight.planned).local().format('YYYY-MM-DDTHH:mm:ss');
		if (flight.estimated)
			flight.estimated = moment.utc(flight.estimated).local().format('YYYY-MM-DDTHH:mm:ss');
		if (flight.actual)
			flight.actual = moment.utc(flight.actual).local().format('YYYY-MM-DDTHH:mm:ss');
